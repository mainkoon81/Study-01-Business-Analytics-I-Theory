...
The Smoothing Parameter α
 -  0 < α < 1
 - If the smoothing parameter is close to 1, more recent observations carry more weight or influence over the forecast (if α = 0.8, weights are 0.8, 0.16, 0.03, 0.01, etc.).
 - If the smoothing parameter is close to 0, the influence or weight of recent and older observations is more balanced (if α = 0.2, weights are 0.2, 0.16, 0.13, 0.10, etc.).

Choosing the Smoothing Parameter α
 - It is often an iterative process. Luckily, advanced statistical tools select the best smoothing parameter based upon **minimizing forecasting error**. Otherwise, you will need to test many smoothing parameters against each other to see which model best fits the data.
